...
export const getInputs = ({ dmmf, config }: { dmmf: DMMF.Document; config: Config }) => {
  ...
  // Its dont used
  const inputsStrings = inputs
    .filter((el) => !el.name.includes('Unchecked'))
    .map((input) => {
      ...
      const fields = (() => {
        ...
        const fieldsString = input.fields
          .map((field) => {
            ...
            const value = (() => {
              if (isScalar) {
                ...
                const removeQuotationMarksFromType = (str: string) =>
                  str.replace(/(type.+:)"(.+)"/, '$1$2');
                const renamedType = (() => {
